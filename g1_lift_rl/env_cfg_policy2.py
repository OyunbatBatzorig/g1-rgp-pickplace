...
from .constants import (
    ...
    PRE_GRASP_ARM_POSE_STD, EE_LINKS, GRASP_OFFSET,
)

# Per-joint Gaussian noise around the verified pre-grasp pose, scaled to how much
# EACH joint actually varies in Policy 1's real convergence (PRE_GRASP_ARM_POSE_STD),
# not a flat +-0.03 rad band on every joint regardless of that joint's real spread.
# Policy 1 won't land at the EXACT same arm configuration every episode, so a
# single fixed pose would only approximate the real hand-off state. If Policy 2
# needs a bit of fine positioning to correct for the residual mismatch, that is a
# reasonable, small part of its own job (reward_descend, kept below at a smaller
# weight than Policy 1's, covers exactly this).
_ARM_POSE_STD = torch.tensor([PRE_GRASP_ARM_POSE_STD[j] for j in ARM_JOINTS])
...
def reset_robot_then_couple_cube(
    ...
    noise = torch.randn(len(env_ids), len(arm_ids), device=env.device) * _ARM_POSE_STD.to(env.device)
