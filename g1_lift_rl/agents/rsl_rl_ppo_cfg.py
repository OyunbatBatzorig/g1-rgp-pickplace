...
@configclass
class G1CombinedPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    """Combined policy (reach+grasp+lift+carry-to-goal, one policy, Franka-lift-
    reference reward design -- env_cfg_combined.py), fresh training from scratch.

    Same architecture/algorithm hyperparameters as Policy 1-4 -- proven stable
    across every run so far, no reason to diverge. max_iterations matches
    Policy 1/3/4's budget (not Policy 2's shorter 800): this single policy has to
    learn strictly more behavior (reach+grasp+lift+goal) than any one link in the
    chain, so it gets the larger budget as a starting point.
    """
    num_steps_per_env = 24
    max_iterations = 1500
    resume = False
    save_interval = 50
    experiment_name = "g1_lift_combined"
    empirical_normalization = False
    ...
    algorithm = RslRlPpoAlgorithmCfg(
        ...
        use_clipped_value_loss=True,
        clip_param=0.2,
        # FIXED (2026-07-27 13:37), REVERTED (2026-07-27 14:31): tried 0.004->0.006
        # (matching Franka's own value) to sustain more exploration, on the theory
        # that G1's monotonically-shrinking action_std (0.60->0.10) was settling
        # too early into a safe local optimum. Combined with num_envs=4096, this
        # produced a WORSE run (deepest crash yet, reach never recovered) -- see
        # env_cfg_combined.py's CurriculumCfg for the actual root cause that
        # result pointed to (curriculum timing, not exploration budget). Reverted
        # to isolate the curriculum-timing fix as the one variable under test.
        entropy_coef=0.004,
        num_learning_epochs=5,
        num_mini_batches=4,
        learning_rate=1.0e-3,
