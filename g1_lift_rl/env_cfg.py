...
G1_DEX1_CFG = ArticulationCfg(
    ...
    actuators={
        ...
        # NEW: waist joints were previously caught by "body"'s broad ".*_joint"
        # match (stiffness=150/damping=10, tuned for arm/contact behavior, not
        # for holding the torso upright) -- since waist isn't in ARM_JOINTS, the
        # policy never commands it, so its target just sits pinned at 0.0 the
        # whole episode, held only by that moderate spring. Measured directly
        # (check_waist_deflection.py against the trained Policy 2 checkpoint,
        # 2026-07-20_16-52-28): under the arm's holding torque while carrying the
        # cube, it settles into a real, sustained several-degree lean --
        # waist_roll -7.9deg (max ~11-12deg), waist_yaw -5.9deg, waist_pitch
        # -2.0deg -- visually confirmed as the "tilted" robot in a screenshot.
        # The root/pelvis is already effectively rigid (0.0000deg deviation
        # measured, check_robot_base_tilt.py) -- this brings the waist to the
        # same standard, consistent with the design intent that only the right
        # arm should move for this manipulation task.
...
class EventCfg:
    ...
    # FIXED: along_range (-0.02,0.02) -> (-0.01,0.01). Checked against the new
    # PRE_GRASP_ARM_POSE/BLOCK_INIT_POS (2026-07-25 teleop): swept the jitter
    # range's corners and compared right_hand_base_link's overlap volume with
    # the cube against the same measurement on the proven-working checkpoint's
    # pose (2026-07-16_09-18-57) as the calibration reference (the raw box
    # test itself over-reports overlap -- rectangular approximation of a
    # thinner real mesh -- so absolute overlap isn't a valid pass/fail bar,
    # only relative-to-a-known-working-pose is). perp_range barely moved the
    # overlap volume; along_range did, sharply and asymmetrically -- at
    # along=+0.02 the overlap volume was ~9-10x the proven pose's, vs ~2x at
    # along=0. Halved to keep the worst-case within roughly the same order of
    # magnitude the working checkpoint already tolerates.
    reset_object = EventTermCfg(
        ...
        params={
            ...
            "along_range": (-0.01, 0.01),  # tightened: along-axis base-plate overlap grew sharply past this
