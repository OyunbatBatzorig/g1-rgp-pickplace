...
# center.
#
# FIXED: (-0.0585,-0.3341) -> (-0.065,-0.349). Matches PRE_GRASP_ARM_POSE's
# update above -- this is the cube spawn position actually present during the
# fresh teleop recording that pose was measured against (arm_teleop.py's
# cube-spawn-tracking, drift 0.0017m confirmed not bumped), not re-derived
# from EE-GRASP_OFFSET arithmetic this time. Z unchanged (still resting on
# the table, TABLE_TOP_Z + BLOCK_SIZE/2).
BLOCK_INIT_POS = (-0.065, -0.349, TABLE_TOP_Z + BLOCK_SIZE / 2.0)  # resting on the table
...
# FIXED: same bug class as INSPECT_ARM_POSE below -- this used to be a solved/
# teleop-derived target Policy 1 was trained to reach, but Policy 1's actual
# converged state (checkpoint 2026-07-16_09-18-57, measured via
# check_policy1_handoff_match.py) diverges from it by up to 0.537 rad
# (right_wrist_pitch) and is outside Policy 2's +-0.03 rad reset noise band on
# 5 of 7 joints -- only 9.4% of Policy 2 resets landed within GRASP_DIST of the
# FIXED (3rd derivation): checkpoint 2026-07-20_13-21-59, retrained against
# CUBE_ROT/BLOCK_INIT_POS's current geometry using this same PRE_GRASP_ARM_POSE
# (still the value measured from 2026-07-16_09-18-57, which predates that
# geometry), failed to converge -- 32-env sweep showed 100% of envs landing
# ~1.7rad away, consistently (a training-dynamics failure, not a bad-geometry
# bug -- FK-checked the old value against the new geometry and it was still
# within ~3.5cm of reward_descend's target). Replaced with a direct teleop
# measurement (arm_teleop.py) against the CURRENT geometry instead of reusing
# a value measured under different conditions. Cube spawn during this
# recording: (-0.065, -0.349, 0.840) -- see BLOCK_INIT_POS below, moved to
# match. NOT yet re-verified by training a checkpoint against it (that's the
# next step) -- this is the teleop ground truth going in, not a confirmed
# result.
PRE_GRASP_ARM_POSE = {
    "right_shoulder_pitch_joint": -0.000,
    "right_shoulder_roll_joint": -0.000,
    "right_shoulder_yaw_joint": +0.250,
...
# Per-joint std of Policy 1's convergence around PRE_GRASP_ARM_POSE above (same
# measurement, check_policy1_handoff_match.py, 32 envs). Policy 2's reset noise
# used to be a flat +-0.03 rad on every joint regardless of how much that joint
# actually varies -- e.g. right_wrist_pitch (std 0.086) genuinely varies ~28x
# more episode-to-episode than right_shoulder_roll (std 0.003), so a uniform
# band either overstates confidence on the noisy joints or wastes exploration
# budget on the stable ones. Used as noise = randn() * PRE_GRASP_ARM_POSE_STD
# instead of the old uniform +-_ARM_POSE_NOISE band.
#
# STALE as of the PRE_GRASP_ARM_POSE update above -- these std values were
# measured around the OLD mean (2026-07-16_09-18-57), not this one. Left as-is
# deliberately: Policy 2 (the only consumer of this constant) is not being
# retrained yet, so there's nothing depending on this being current right now.
# Before Policy 2's next retrain, re-measure fresh against whatever checkpoint
# this new PRE_GRASP_ARM_POSE actually produces -- same methodology, don't
# just assume these numbers still apply.
PRE_GRASP_ARM_POSE_STD = {
    "right_shoulder_pitch_joint": 0.0443,
    "right_shoulder_roll_joint": 0.0028,
    "right_shoulder_yaw_joint": 0.0328,
    "right_elbow_joint": 0.0207,
    "right_wrist_roll_joint": 0.0339,
    "right_wrist_pitch_joint": 0.0858,
    "right_wrist_yaw_joint": 0.0422,
}
