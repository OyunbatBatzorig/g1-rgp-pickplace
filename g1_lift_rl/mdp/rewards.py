...
# ---------------- Combined policy (single-policy, Franka-lift-reference-style) ----------------
# Reward design ported from IsaacLab's own reference task (manipulation/lift/mdp/
# rewards.py, Franka arm) for G1LiftCombinedEnvCfg (env_cfg_combined.py) -- a single
# policy doing reach+grasp+lift+carry-to-goal, as a comparison point against the
# 4-policy chain above. Reuses _ee_obj_dist/_cube_pos_w (already defined above), no
# new geometry/body-lookup code.
REACH_STD = 0.3          # FIXED (2026-07-27 10:15): was 0.1, matching the reference's own literal value --
                         # but measured directly (view_franka_setup.py vs the combined-
                         # policy test-training run) that this only makes sense for
                         # Franka's own reset geometry, not G1's. Franka's default/ready
                         # joint config starts the gripper 0.393m from the cube (dist/std=
                         # 3.93 at std=0.1); G1's READY_ARM_POSE (chosen early in this
                         # project for table-clearance safety, not reach convenience)
                         # starts ~0.55m away (dist/std=5.55). The tanh-kernel's GRADIENT,
                         # not just its value, collapses steeply with dist/std --
                         # sech^2(3.93)=0.0015 vs sech^2(5.55)=0.00006, a ~25x weaker
                         # signal at G1's actual starting distance, on top of G1's lower
                         # init_noise_std (0.6 vs Franka's 1.0) and half the envs (2048 vs
                         # 4096) giving less exploration to compensate. Confirmed empirically:
                         # a 300-iteration test run at std=0.1 showed Episode_Reward/
                         # reaching_object flat at 0.0000/0.0001 for every single iteration,
                         # no trend at all -- not an undertraining symptom, a near-zero-
                         # gradient symptom. 0.3 puts G1's own reset at dist/std=1.85,
                         # gradient~0.31 -- stronger than Franka's own effective gradient at
                         # ITS reset, giving PPO a real hill to climb from step 0.
GOAL_STD_COARSE = 0.3    # matches reference object_goal_tracking
GOAL_STD_FINE = 0.05     # matches reference object_goal_tracking_fine_grained
# Re-based from the reference's minimal_height=0.04 -- that value is ABSOLUTE
# world-frame z in the reference scene (table near z=0 there); this scene's table
# top sits at TABLE_TOP_Z=0.81, so the same literal 0.04 would make this gate
# permanently true/false depending on sign. Anchored onto _CUBE_REST_Z instead,
# reusing the reference's own 0.04 delta.
LIFT_MINIMAL_HEIGHT = _CUBE_REST_Z + 0.04


def reward_reach(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Tanh-kernel reach reward -- matches reference object_ee_distance."""
    return 1.0 - torch.tanh(_ee_obj_dist(env) / REACH_STD)


def reward_object_lifted(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Binary bonus once the cube crosses LIFT_MINIMAL_HEIGHT -- matches reference
    object_is_lifted, PLUS an added _is_grasping() gate (see FIXED note below)."""
    lifted = _cube_pos_w(env)[:, 2] > LIFT_MINIMAL_HEIGHT
    # FIXED (2026-07-27 10:50): the reference's own height-only check has no notion of
    # WHY the cube is high -- a deliberate lift and a random smack/collision both
    # satisfy it identically. A 300-iteration test run on G1 exploited exactly
    # this: Episode_Termination/dropped climbed to 79% and launched to 16% of all
    # episodes by the end, moving in lockstep with lifting_object's own reward
    # climbing (0.28->0.58) as reaching_object's reward SIMULTANEOUSLY declined
    # (0.22 peak -> 0.06) -- the policy learned to whack the cube up instead of
    # reaching+grasping it, since a smack pays the same as a real lift here.
    # Confirmed via a deterministic single-episode replay (diagnose_combined_
    # checkpoint.py, checkpoint 2026-07-27_10-20-28/model_299): cube launched to
    # peak velocity 3.187 m/s with is_grasping=False for 100% of the 45 logged
    # steps -- never once actually held. Gated on _is_grasping() (EE within
    # GRASP_DIST of the cube AND gripper closed past GRIP_CLOSED_THRESHOLD --
    # the same helper g1_lift_ext's own 4-policy chain already uses for
    # reward_lift/reward_inspect, for this identical reason) so a smack with an
    # open/uncommitted gripper can no longer collect this reward at all.
    # reward_reach (the dense approach pull) is deliberately NOT gated -- it
    # should keep firing during the approach itself, only the lift/goal rewards
    # needed the added grasp requirement.
    return (lifted & _is_grasping(env)).float()


def _goal_tracking(env: ManagerBasedRLEnv, std: float) -> torch.Tensor:
    goal_w = torch.tensor(INSPECT_POS, device=env.device) + env.scene.env_origins
    dist = torch.norm(_cube_pos_w(env) - goal_w, dim=-1)
    # FIXED (2026-07-27 10:50): same _is_grasping() gate as reward_object_lifted above,
    # same reasoning -- was gated on the height check alone, which a smack/launch
    # satisfies just as well as a genuine held-and-carried cube.
    lifted_and_grasping = (_cube_pos_w(env)[:, 2] > LIFT_MINIMAL_HEIGHT) & _is_grasping(env)
    return lifted_and_grasping.float() * (1.0 - torch.tanh(dist / std))


def reward_goal_tracking_coarse(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Matches reference object_goal_tracking (std=0.3)."""
    return _goal_tracking(env, GOAL_STD_COARSE)


def reward_goal_tracking_fine(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Matches reference object_goal_tracking_fine_grained (std=0.05)."""
    return _goal_tracking(env, GOAL_STD_FINE)


# NEW (2026-07-27 17:04): ungated variants -- the reference's LITERAL formula,
# height-only, no _is_grasping() gate -- for env_cfg_g1_franka_parity.py's
# arm-swap ablation. That experiment's whole point is testing G1's real arm
# under the EXACT SAME reward/curriculum treatment the Franka-Far/FarFixed/
# SoftGripper/Small chain used (none of which had the grasp-gate -- it's a
# g1_lift_combined-specific fix, added AFTER those ablations were already
# running unmodified). Using the gated versions here would reintroduce a
# reward-design confound into what's supposed to be a pure "only the robot
# differs" comparison. Deliberately separate functions, not a flag on the
# existing ones -- g1_lift_combined's own gated versions stay untouched.
def reward_object_lifted_ungated(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Reference object_is_lifted, literally -- height-only, no grasp gate."""
    return (_cube_pos_w(env)[:, 2] > LIFT_MINIMAL_HEIGHT).float()


def _goal_tracking_ungated(env: ManagerBasedRLEnv, std: float) -> torch.Tensor:
    goal_w = torch.tensor(INSPECT_POS, device=env.device) + env.scene.env_origins
    dist = torch.norm(_cube_pos_w(env) - goal_w, dim=-1)
    lifted = (_cube_pos_w(env)[:, 2] > LIFT_MINIMAL_HEIGHT).float()
    return lifted * (1.0 - torch.tanh(dist / std))


def reward_goal_tracking_coarse_ungated(env: ManagerBasedRLEnv) -> torch.Tensor:
    return _goal_tracking_ungated(env, GOAL_STD_COARSE)


def reward_goal_tracking_fine_ungated(env: ManagerBasedRLEnv) -> torch.Tensor:
    return _goal_tracking_ungated(env, GOAL_STD_FINE)
...
def reward_descend(env: ManagerBasedRLEnv) -> torch.Tensor:
    ...
    #
    # CONSIDERED (while diagnosing checkpoint 2026-07-20_13-21-59, which pushed
    # the cube instead of converging) and REVERTED: rotating GRASP_OFFSET by
    # the cube's own orientation here, reasoning it should be cube-local like
    # reset_object_cube_frame_jitter's jitter axes. Wrong -- GRASP_OFFSET is
    # used as a fixed WORLD-frame vector everywhere else in this codebase
    # (BLOCK_INIT_POS's derivation, INSPECT_POS's derivation, Policy 2/3/4's
    # reset events), and CUBE_ROT's own comment history confirms it was
    # specifically tuned so that THIS fixed direction lines up with a cube
    # face -- CUBE_ROT was chosen to match GRASP_OFFSET, not the reverse.
    # Rotating it only here would have broken that alignment and made this
    # function inconsistent with every other GRASP_OFFSET usage. Not the bug.
...
# NEW: catches slow cumulative cube drag during Policy 4's return-to-ready retreat
# that penalty_contact_disturbance's velocity gate misses entirely. Found via
# chained_rollout end-to-end diagnostics against the validated checkpoints
# (Policy1 2026-07-16_09-18-57, Policy2 2026-07-11_12-00-16, Policy3
# 2026-07-16_12-23-10, Policy4 2026-07-16_13-30-32): 2 of 6 successfully-placed
# envs had the cube dragged 12-20cm away from GOAL_POS during Policy 4's own
# retreat, despite ending within 3-10mm of the goal right after Policy 3. A
# 12-20cm drift spread over the ~8s (795-step) return only needs to average
# ~0.015-0.03 m/s -- well under DISTURBANCE_VEL_THRESHOLD=0.05m/s at any single
# step, so a policy can learn a slow, steady drag that never trips the velocity
# gate even once, while still wrecking the placement by episode end. Root cause:
# penalty_contact_disturbance is a per-step velocity THRESHOLD (local), not a
# cumulative POSITION penalty (global) -- this term is the latter.
def penalty_cube_drift_after_release(env: ManagerBasedRLEnv) -> torch.Tensor:
    """Penalizes the cube's XY distance from GOAL_POS while not grasping (same
    gate as reward_return_to_ready, so it only applies post-release, not during
    the approach/carry phase -- carrying the cube toward the goal is the whole
    point of Policy 3's job and must not be penalized here). Zero within
    PLACE_DIST (a cube still "at the goal" isn't penalized for sub-threshold
    jitter, consistent with _at_goal_settled's own tolerance); grows linearly
    in meters beyond it, so any accumulated drift -- fast or slow -- is
    penalized every single step it persists, not just the step it happens on."""
    obj: RigidObject = env.scene["object"]
    goal_w = torch.tensor(GOAL_POS, device=env.device) + env.scene.env_origins
    d_xy = torch.norm(obj.data.root_pos_w[:, :2] - goal_w[:, :2], dim=-1)
    excess = torch.clamp(d_xy - PLACE_DIST, min=0.0)
    not_grasping = (~_is_grasping(env)).float()
    return excess * not_grasping
