...
class RewardsCfg:
    ...
    # NEW: penalty_contact_disturbance is velocity-only and misses slow cumulative
    # drag during retreat (see its own docstring in rewards.py for the measured
    # 12-20cm drift this catches that the velocity gate didn't). Weight -5.0 puts
    # a 10cm drift event at -0.5/step, same order of magnitude as
    # return_to_ready's max +1.0/step -- meaningful, not overwhelming.
    cube_drift = RewTerm(func=mdp.penalty_cube_drift_after_release, weight=-5.0)
