...
CUBE SPAWN TRACKING: the cube's position right after each reset (initial, and
any drop-triggered re-reset) is captured and printed once immediately, then
carried into every status line and recorded/printed readout as "cube_spawn" +
a drift-from-spawn distance, with a "[BUMPED?]"/"LIKELY BUMPED" flag once that
drift exceeds 5mm -- so an accidental knock during teleop shows up live
instead of only being discoverable (or not) after the fact.
...
def main():
    ...
    # Captured once right after each reset (initial + any drop-triggered
    # re-reset), BEFORE any arm movement can touch it -- printed on capture,
    # and carried into every readout below so a later recording can always be
    # checked against where the cube actually started, not just where it
    # ended up. Motivated directly by a past session where the cube got
    # bumped mid-teleop with no record of its original spawn point to notice
    # or correct against.
    cube_spawn_pos = None

    def capture_cube_spawn(reason):
        nonlocal cube_spawn_pos
        cube_spawn_pos = (scene["object"].data.root_pos_w[0] - scene.env_origins[0]).tolist()
        print(f"\n[CUBE SPAWN] ({reason}) cube respawned at "
              f"({cube_spawn_pos[0]:+.3f}, {cube_spawn_pos[1]:+.3f}, {cube_spawn_pos[2]:+.3f})\n")
    ...
    def gather_readout(label=None):
        ...
        cube_drift_from_spawn = (
            torch.norm(cube - torch.tensor(cube_spawn_pos, device=cube.device)).item()
            if cube_spawn_pos is not None else 0.0
        )
        ...
        return {
            ...
            "cube": cube.tolist(),
            "cube_spawn": cube_spawn_pos,
            "cube_drift_from_spawn": cube_drift_from_spawn,
            "dist_mid": d_mid,
            "dist_f1": d1,
            "dist_f2": d2,
    ...
    def status():
        ...
        # BUMPED flag: cheap, always-visible check for exactly the failure
        # mode this was added for -- the cube drifting from its recorded
        # spawn point (a bump) without it being noticed until later.
        bump_flag = " [BUMPED?]" if r["cube_drift_from_spawn"] > 0.005 else ""
        print(f"\r  EE({r['ee_mid'][0]:+.3f},{r['ee_mid'][1]:+.3f},{r['ee_mid'][2]:+.3f})"
              ...
              f" Δspawn={r['cube_drift_from_spawn']:.3f}{bump_flag}"
    ...
    def print_readout(r, header):
        ...
        if r["cube_spawn"] is not None:
            print(f"Cube spawn position:    ({r['cube_spawn'][0]:+.3f}, {r['cube_spawn'][1]:+.3f}, {r['cube_spawn'][2]:+.3f})")
        print(f"Cube position (actual): ({r['cube'][0]:+.3f}, {r['cube'][1]:+.3f}, {r['cube'][2]:+.3f})")
        if r["cube_spawn"] is not None:
            bumped = " <-- LIKELY BUMPED, not at its spawn position" if r["cube_drift_from_spawn"] > 0.005 else " (matches spawn)"
            print(f"Cube drift from spawn:  {r['cube_drift_from_spawn']:.4f} m{bumped}")
    ...
    def check_cube_dropped():
        ...
        if cube_z < DROP_HEIGHT_THRESHOLD:
            ...
            env.reset()
            capture_cube_spawn("after drop-triggered reset")
            return True
        return False
    ...
    print(__doc__)
    env.reset()
    capture_cube_spawn("initial reset")
    try:
