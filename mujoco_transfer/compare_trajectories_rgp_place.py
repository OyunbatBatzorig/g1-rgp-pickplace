"""Compares RGP Policy 3's (place) Isaac Lab rollout against its MuJoCo
rollout, both fed the SAME exported policy.pt and the SAME fixed reset state
(PLACE_ARM_POSE/PLACE_GRIPPER_POSE/PLACE_CUBE_POS in run_rgp_place.py, matching
policy2_held_states.pt's fixed_idx pick used by Isaac Lab's own reset event) --
isolating physics/geometry reconstruction differences as the source of any gap.

Same 39-dim obs layout as compare_trajectories_rgp_grasp.py, plus a
dist-to-goal comparison specific to this policy (Policy 1/2 have no goal
concept).
"""
import numpy as np
import matplotlib
...
isaac = np.load("trajectory_isaac_rgp_place.npz")
mjc = np.load("trajectory_mujoco_rgp_place.npz")
...
print(f"arm start pose gap (isaac vs mujoco, rad): {np.linalg.norm(arm_i[0] - arm_m[0]):.4f}  "
      f"(should be ~0 -- both start from the same fixed PLACE_ARM_POSE by construction)")
...
plot_pair(axes[0, 1], object_to_goal_i, object_to_goal_m, "Cube-to-goal distance (settle radius=0.06m)", "m")
...
fig.tight_layout()
fig.savefig("trajectory_comparison_rgp_place.png", dpi=130)
print("\nsaved trajectory_comparison_rgp_place.png")
...
axes2[7].axis("off")
fig2.tight_layout()
fig2.savefig("trajectory_comparison_rgp_place_joints.png", dpi=130)
print("saved trajectory_comparison_rgp_place_joints.png")
