"""Runs Policy 3's trained checkpoint (2026-07-16_13-18-22: move to goal +
place) in MuJoCo.

UNLIKE Policy 1/2, Policy 3's ObservationsCfg is only 7 terms / 33-dim, not
9/39 -- it drops object_to_inspect and hand_base_to_object (env_cfg_policy3.py):
arm_joint_pos_rel(7), arm_joint_vel(7), gripper_joint_pos(2), object_position(3),
ee_position(3), ee_to_object(3), last_action(8) = 33. build_observation() below
reflects that, not a copy of Policy 1/2's.

STARTING STATE: env_cfg_policy3.py's reset_robot_then_couple_cube resets the
arm to INSPECT_ARM_POSE (Policy 2's measured convergence) with the gripper at
INSPECT_GRIP_VALUE (the real, mechanically-blocked grip value, ~-0.0144 --
NOT the unreachable full-closure GRIPPER_CLOSE), then couples the cube to the
resulting actual EE position minus GRASP_OFFSET. Replicated exactly here.
INSPECT_GRIP_VALUE is an intermediate (non-binary) isaac-space gripper value,
so this needs the general lerp-based conversion (to_mjc_gripper_continuous
below), not the binary to_mjc_gripper Policy 1/2 use (which only ever
receives exactly GRIPPER_OPEN or GRIPPER_CLOSE as input).
"""
import argparse
import sys
import time
...
parser.add_argument("--policy_path", type=str,
                     default="/home/virtual-acc/projects/g1_lift_ext/logs/rsl_rl/g1_policy3_place_release/2026-07-16_13-18-22/exported/policy.pt")
...
parser.add_argument("--arm_pose", type=float, nargs=7, default=None,
                     help="override the 7 ARM_JOINTS starting values, matching Isaac Lab's actual sampled reset pose")
args = parser.parse_args()

GRIPPER_CLOSE = 0.0245
GRASP_OFFSET = (-0.010, 0.027, 0.012)
GOAL_POS = np.array([0.070, -0.389, 0.846])

INSPECT_ARM_POSE_AT_TRAIN_TIME = {
...
# 1. Arm -> INSPECT_ARM_POSE_AT_TRAIN_TIME (or --arm_pose override).
start_pose = dict(INSPECT_ARM_POSE_AT_TRAIN_TIME)
...
def to_mjc_gripper(isaac_value):
    """Binary conversion (action-space): isaac_value is always exactly
    GRIPPER_OPEN or GRIPPER_CLOSE here."""
...
def to_mjc_gripper_continuous(isaac_value):
    """General conversion for an ARBITRARY isaac-space value (e.g.
    INSPECT_GRIP_VALUE, a real mechanically-blocked grip position, not one of
    the two binary endpoints) -- inverts to_isaac_gripper's lerp chain."""
...
# 2. Gripper -> INSPECT_GRIP_VALUE (continuous conversion), THEN couple cube
# to the resulting actual EE position minus GRASP_OFFSET.
gripper_start_mjc = to_mjc_gripper_continuous(INSPECT_GRIP_VALUE)
d.qpos[GRIPPER_MJC_IDS] = gripper_start_mjc
d.ctrl[GRIPPER_MJC_IDS] = gripper_start_mjc
mujoco.mj_forward(m, d)
...
def build_observation():
    """Policy 3's 33-dim obs: NO object_to_inspect, NO hand_base_to_object."""
...
print(f"Policy 3 checkpoint: {args.policy_path}")
...
for t in range(args.steps):
    ...
    goal_dist = float(np.linalg.norm(GOAL_POS - object_pos))
    dist_history.append(dist)
    goal_dist_history.append(goal_dist)
    if t % args.log_every == 0 or t == args.steps - 1:
        print(f"  step {t:4d}  EE-object dist={dist:.4f}  object-to-goal={goal_dist:.4f}  "
...
print(f"final object-to-goal dist: {goal_dist_history[-1]:.4f}")
print(f"min object-to-goal dist over episode: {min(goal_dist_history):.4f}")
