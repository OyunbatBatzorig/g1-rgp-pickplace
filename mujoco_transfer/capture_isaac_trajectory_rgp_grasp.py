"""Captures RGP Policy 2's (grasp+lift) deterministic Isaac Lab rollout (obs +
action + absolute arm joint positions, per control step) to an .npz file, for
direct comparison against run_rgp_grasp.py's MuJoCo rollout.

Same 36-dim obs layout as Policy 1 (mdp/observations_rgp.py is shared across
the chain), gym task Isaac-G1-RGP-Grasp-Play-v0. One extra thing Policy 1
didn't need: Policy 2's reset samples a RANDOM arm pose around
RGP_POLICY1_ARM_POSE (mean +- measured std, see mdp/events_rgp.py) -- so this
script also logs the ACTUAL sampled arm pose at t=0, to pass to
run_rgp_grasp.py's --arm_pose so both sims start from the identical state
(otherwise a transfer-gap comparison would be confounded by different starting
poses, not just different physics).
"""
import argparse
...
parser.add_argument("--out", type=str,
                     default="/home/virtual-acc/projects/g1_lift_ext/mujoco_transfer/trajectory_isaac_rgp_grasp.npz")
...
task = "Isaac-G1-RGP-Grasp-Play-v0"
...
print(f"arm_pose0 (pass to run_rgp_grasp.py --arm_pose): {' '.join(f'{v:.6f}' for v in arm_pose0)}")
