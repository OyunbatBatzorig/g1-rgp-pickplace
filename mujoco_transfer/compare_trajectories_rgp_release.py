"""Compares RGP Policy 4's (release + return-to-ready) Isaac Lab rollout
against its MuJoCo rollout, both fed the SAME exported policy.pt and the SAME
fixed reset state (RELEASE_ARM_POSE/RELEASE_GRIPPER_POSE/RELEASE_CUBE_POS in
run_rgp_release.py, matching policy3_settled_states.pt's fixed_idx pick used
by Isaac Lab's own reset event) -- isolating physics/geometry reconstruction
differences as the source of any gap.

Same 39-dim obs layout as compare_trajectories_rgp_place.py, plus an
arm-distance-to-READY_ARM_POSE comparison specific to this policy (the
return-to-ready behaviour Policy 1/2/3 have no equivalent of).
"""
import numpy as np
import matplotlib
matplotlib.use("Agg")
...
READY_ARM_POSE = np.array([0.60, -0.20, 0.0, 0.80, 0.0, 0.40, 0.0])  # Policy 1's own reset pose

isaac = np.load("trajectory_isaac_rgp_release.npz")
mjc = np.load("trajectory_mujoco_rgp_release.npz")
...
print(f"arm start pose gap (isaac vs mujoco, rad): {np.linalg.norm(arm_i[0] - arm_m[0]):.4f}  "
      f"(should be ~0 -- both start from the same fixed RELEASE_ARM_POSE by construction)")

# obs layout: arm_joint_pos_rel[0:7] arm_joint_vel[7:14] gripper_joint_pos[14:16]
# object_position[16:19] ee_position[19:22] ee_to_object[22:25]
# hand_base_to_object[25:28] object_to_goal[28:31] last_action[31:39]
ee_to_object_i = np.linalg.norm(obs_i[:, 22:25], axis=1)
ee_to_object_m = np.linalg.norm(obs_m[:, 22:25], axis=1)
object_to_goal_i = np.linalg.norm(obs_i[:, 28:31], axis=1)
object_to_goal_m = np.linalg.norm(obs_m[:, 28:31], axis=1)
...
arm_dist_ready_i = np.linalg.norm(arm_i - READY_ARM_POSE, axis=1)
arm_dist_ready_m = np.linalg.norm(arm_m - READY_ARM_POSE, axis=1)
...
for name, i_arr, m_arr in [
    ...
    ("dist_to_goal (m)", object_to_goal_i, object_to_goal_m),
    ("gripper_joint_pos (mean)", gripper_i, gripper_m),
    ("arm_action_norm", arm_action_norm_i, arm_action_norm_m),
    ("arm_dist_to_ready (rad)", arm_dist_ready_i, arm_dist_ready_m),
]:
...
plot_pair(axes[0, 1], arm_dist_ready_i, arm_dist_ready_m, "Arm distance to READY_ARM_POSE", "rad")
...
plot_pair(axes[2, 0], arm_action_norm_i, arm_action_norm_m, "Arm action norm (raw policy output)", "-")
plot_pair(axes[2, 1], object_to_goal_i, object_to_goal_m, "Cube-to-goal distance", "m")

fig.tight_layout()
fig.savefig("trajectory_comparison_rgp_release.png", dpi=130)
print("\nsaved trajectory_comparison_rgp_release.png")
...
axes2[7].axis("off")
fig2.tight_layout()
fig2.savefig("trajectory_comparison_rgp_release_joints.png", dpi=130)
print("saved trajectory_comparison_rgp_release_joints.png")
